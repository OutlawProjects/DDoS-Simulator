# DDoS Sim
#### A project by Outlaw



# Features
### With DDoS-Sim tool, you can:
- Take a view in a fake DDoS Panel 
- Learn about ports & website ips

Anyone may use this tool for educational purposes.

# Installation Guide
## Requirements: 
- Linux OS / Mac OS
- Bash installed ( Usually pre-installed ).

## Installation Guide
#### Clone repository:
```bash
git clone https://github.com/sadsism/DDoS-Simulator
cd DDoS-Simulator
```
#### Make script excecutable:
```bash
chmod +x ddos-sim.sh
```
#### Run script:
```bash
./ddos-sim.sh
```
## Controls
- CTRL + C: Kill running simulation
- ENTER: Return to main menu
- BACKSPACE: Exit

## Disclaimer
This script does not send real packages. This script is a harmless DDoS Panel simulator. I am not responsible for any harm caused by my script. 

## Support
You support me by just starring this repository ⭐️, or donating to my [BuyMeACoffee](https://buymeacoffee.com/sadsadist) 🙏
